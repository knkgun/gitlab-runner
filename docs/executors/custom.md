...
| `job_env` | object | ✗ | ✓ |  Name-value pairs that will be made available through environment variables to all subsequent stages of the job execution. They are made available for the driver, not the job - please read [the details bellow](#job_env-usage). |
...
The main purpose of `job_env` configuration is to pass variables **to the context of custom executor drier calls**
...
With Custom Executor execution flow - where each job execution stage: `prepare`, multiple `run` calls
...
If the only purpose of why you're looking at `job_env` setting is passing a set of variable to every job executed
by a particular Runner, then the
[`environment` setting from `[[runners]]`](../configuration/advanced-configuration.md#the-runners-section) is what you
should look at.
